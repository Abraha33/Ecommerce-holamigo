...
import { ShoppingCart, X, Plus, Minus, ArrowRight, Lock, Unlock } from "lucide-react"
...
export function PersistentCartSidebar() {
  ...
  return (
    <>
      {/* Cart toggle button - outside the sidebar */}
      <div
        className={`fixed right-4 top-20 z-40 transition-all duration-300 ${
          isExpanded || isHovered || isLocked ? "opacity-0 translate-x-10" : "opacity-100"
        }`}
      >
        <button
          onClick={() => {
          ...
          className="bg-[#004a93] text-white p-3 rounded-full shadow-lg flex items-center justify-center hover:bg-[#0071bc] transition-all"
      ...
      {/* Lock button - outside the sidebar */}
      <div
        className={`fixed right-4 top-32 z-40 transition-all duration-300 ${
          isExpanded || isHovered || isLocked ? "opacity-0 translate-x-10" : "opacity-100"
        }`}
      >
        <button
          ...
          className={`p-3 rounded-full shadow-lg flex items-center justify-center transition-all ${
            isLocked ? "bg-[#e30613] text-white" : "bg-white text-[#004a93] border border-[#004a93]"
          }`}
          title={isLocked ? "Desbloquear carrito" : "Bloquear carrito"}
        >
          {isLocked ? <Lock className="h-5 w-5" /> : <Unlock className="h-5 w-5" />}
        </button>
      </div>

      {/* Visual indicator for opening/closing */}
      <div
        className={`fixed right-0 top-0 bottom-0 w-1 bg-[#004a93] transition-all duration-500 ${
          isTransitioning ? "opacity-100 w-3" : "opacity-0"
        }`}
      />

      <div
        className={`fixed right-0 top-0 bottom-0 z-40 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isExpanded || isHovered || isLocked ? "translate-x-0 shadow-2xl" : "translate-x-full"
        } ${isTransitioning ? "scale-[1.02] transition-all duration-300" : ""}`}
        onMouseEnter={() => {
          if (!isLocked) {
            setIsHovered(true)
            setIsTransitioning(true)
      ...
      >
        ...
        {/* Cart content */}
        <div className="bg-white border-l border-gray-200 h-full w-64 shadow-[0_0_25px_rgba(0,0,0,0.15)] flex flex-col relative transition-all duration-300 ease-in-out">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`p-1.5 rounded-full transition-colors ${
                    isLocked ? "bg-[#e30613] text-white" : "hover:bg-[#0071bc]"
                  }`}
                  title={isLocked ? "Desbloquear carrito" : "Bloquear carrito"}
                >
                  {isLocked ? <Lock className="h-4 w-4 animate-pulse" /> : <Unlock className="h-4 w-4" />}
                </button>
                <button
                  onClick={() => {
                    setIsExpanded(false)
                  ...
                  className="p-1.5 hover:bg-[#0071bc] rounded-full transition-colors"
          ...
          {(isLoading || isUpdating) && (
            <div className="absolute inset-0 bg-white/70 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="flex flex-col items-center">
                ...
                <span className="mt-4 text-[#004a93] font-medium">
                  {isLoading ? "Cargando carrito..." : "Actualizando..."}
                </span>
              </div>
            </div>
          )}
